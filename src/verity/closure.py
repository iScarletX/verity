...
def evaluate_v1_closure(*, engineering_checks: Dict[str, bool],
    ...
    deferred = [
        {"code": "v1_5_prompt_blackbox_not_enabled_by_default",
         "detail": ("Integrated into review.run_review (verity.blackbox.config."
                    "BlackboxConfig) since Round 74, but OFF by default and "
                    "requires an explicit caller-supplied enabled config with a "
                    "trusted model Provider; the reviewed prompt can never turn "
                    "it on itself. Not counted as a V1 engineering failure -- "
                    "the deterministic release scope does not require it.")},
        {"code": "v2_skill_sandbox_not_enabled_by_default",
         "detail": ("Integrated into review.run_review (verity.sandbox.config."
                    "SandboxConfig) since Round 74, but OFF by default and "
                    "requires an explicit caller-supplied enabled config naming "
                    "an entry point; the reviewed Skill can never turn it on "
                    "itself. Not counted as a V1 engineering failure -- the "
                    "deterministic release scope does not require it.")},
