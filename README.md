...
the check; it does not mean complete recall or evaluated accuracy. Independent
labels and paired real observations remain mandatory before the comparator can
emit a scoped superiority claim. A
historical v1
...
| `skill.upstream_dependency_contract_gap` | medium | — | SKILL.md body declares upstream Skill outputs as prerequisites in prose (e.g. "仅在X已产出"), but manifest `dependencies` and `metadata` are both empty — no machine-checkable upstream contract exists. |
| `skill.scope_restrictions_prose_only` | low | — | SKILL.md body declares "do not use for X" scope restrictions in prose only, with no corresponding `allowed-tools`/`permissions` manifest fields. Informational; restrictions rely solely on model instruction-following. |
| `skill.no_fabrication_declared` | low | — | SKILL.md body declares a no-fabrication mandate ("不得虚构"/"不得脑补") in prose only, with no corresponding `permissions`/`metadata` manifest field. |
| `skill.strict_output_contract_prose_only` | low | — | SKILL.md body declares a strict single-JSON-object output contract in prose only, with no manifest `refs` entry pointing at a schema file. |
| `skill.tool_unavailable_contract_prose_only` | low | — | SKILL.md body declares a Tool-unavailable fallback in prose only, with no machine-readable fallback declaration in manifest `metadata`. |
| `skill.missing_inline_reference` | medium | AST04 | SKILL.md body contains an inline `references/<file>` link pointing to a file absent from the Skill package. Complements `skill.manifest_missing_reference`, which only checks the YAML `refs` field. |
| `skill.business_interface_version_gap` | medium | — | SKILL.md body references `references/business-interface.md` without naming the same contract version string declared in that file. |
| `skill.runtime_state_file_malformed` | medium | — | `current.json` or `history.jsonl` is present but empty or fails to parse as valid JSON/JSONL. |
| `skill.bandit_finding` (rules `skill.bandit.<test_id>`) | varies | AST01/AST02/AST05 depending on test | 15 curated Bandit test_ids: B102 (exec) / B105/B106/B107 (hardcoded passwords) / B301 (pickle load) / B310 (unsafe urlopen) / B314 (unsafe XML parser, e.g. XXE) / B324 (weak MD4/MD5/SHA1 hash) / B501 (TLS certificate verification disabled) / B506 (yaml.load unsafe) / B602 (subprocess shell=True) / B605 (os.system) / B607 (partial exec path) / B608 (SQL query string concatenation) / B701 (jinja2 autoescape). Bandit's `issue_text` never contributes to identity; Verity's severity is the policy value, not Bandit's raw severity. |
...
| `prompt.control_character` | medium | any | ASCII control characters (except \t, \n, \r), Unicode bidi overrides (U+202A–U+202E, U+2066–U+2069), and invisible/zero-width chars (ZWSP/ZWNJ/ZWJ/word-joiner/BOM) + the Unicode TAG block U+E0000–E007F (tag-smuggling), reported as `invisible_char`. NUL is rejected at intake. Invisible-char coverage adapted from ProtectAI llm-guard invisible_text. |
...
| `prompt.untrusted_input_boundary_undeclared` | medium | `system_prompt` only | Declares acceptance of external/user-supplied content with no trust-boundary or anti-injection-override statement anywhere in the document. Round 52: acceptance detection is now a multi-signal co-occurrence gate (ingestion-verb + content-object + provenance, English/Chinese) that fires on realistic phrasings of ingesting rich/third-party content (documents, emails, files, attachments, tickets, retrieved/tool content) and deliberately stays silent on generic conversational Q&A ("answer the user's question"). Still cannot judge whether a present mitigation is actually effective. Maps to VR-PROMPT-008. |
