"""Round 98: close VR-SKILL-002's L1_semantic=none gap with a new Finding
Type, semantic.skill.path_traversal_input_trust_gap.
"""
from verity.intake import intake_directory
from verity.report import review_to_dict
from verity.review import ReviewInputs, run_review
from verity.semantic.catalog import CATALOG, extract_path_traversal_input_trust_gap
from verity.semantic_benchmark import BUTLER_REFERENCE_SKILLS
from verity.standards import (load_detector_mappings, load_risks,
                              validate_runtime_detector_coverage)

FINDING_TYPE = "semantic.skill.path_traversal_input_trust_gap"
RISK_ID = "VR-SKILL-002"
...
def test_catalog_has_new_finding_type_with_expected_shape():
    ...
    assert definition.subjectKeyFields == ["pathTrustGapKind"]
    assert extractor is extract_path_traversal_input_trust_gap


def test_detector_mapping_targets_vr_skill_002_as_signal():
...
def test_vr_skill_002_l1_semantic_coverage_is_now_signal():
    risks = load_risks()
...
def test_extractor_seeds_with_candidate_hint_on_user_controlled_input_framing(tmp_path):
    root = tmp_path / "path-traversal-user-input"
    root.mkdir()
    (root / "SKILL.md").write_text(
        "---\nname: path-traversal-user-input\n"
        "description: Reads a report file using the user-provided "
        "filename submitted through the upload form.\n"
        "allowed-tools: Read\n---\n")
    ...
    seeds = extract_path_traversal_input_trust_gap(review_to_dict(review), data)
    assert seeds
    source, _evidence_ids, evidence = seeds[0]
    assert source["pathReferenceFactCount"] >= 1
    assert source["candidateHints"]
    assert source["candidateHints"][0]["subject"]["pathTrustGapKind"] == (
        "user_controlled_path_reference")
    paths = {loc["artifactPath"] for ev in evidence for loc in ev["locations"]}
    assert paths == {"SKILL.md", "run.py"}


def test_extractor_skips_candidate_hint_on_safe_path_reference_framing(tmp_path):
    root = tmp_path / "path-traversal-fixed"
    root.mkdir()
    (root / "SKILL.md").write_text(
        "---\nname: path-traversal-fixed\n"
        "description: Reads a report file using a fixed path restricted "
        "to the package-relative reports directory.\n"
        "allowed-tools: Read\n---\n")
    (root / "run.py").write_text(
        "filename = 'report.txt'\n"
        "with open(f'/data/reports/{filename}') as handle:\n"
...
def test_extractor_does_not_seed_without_a_path_fact(tmp_path):
    root = tmp_path / "no-dynamic-path"
    root.mkdir()
    (root / "SKILL.md").write_text(
...
def test_extractor_does_not_seed_a_static_path_with_no_dynamic_construction(tmp_path):
    root = tmp_path / "static-path"
    root.mkdir()
    (root / "SKILL.md").write_text(
        "---\nname: static-path\n"
        "description: Counts the report files in a fixed directory.\n"
        "allowed-tools: Read\n---\n")
    (root / "run.py").write_text(
        "with open('/data/reports/report.txt') as handle:\n"
        "    text = handle.read()\n")
    snap, data = intake_directory(root)
    review = run_review(ReviewInputs("skill", snap, data, profile="minimal"))
    seeds = extract_path_traversal_input_trust_gap(review_to_dict(review), data)
...
def test_extractor_is_skill_engine_only():
    assert extract_path_traversal_input_trust_gap(
        {"engine": "prompt", "snapshot": {"files": []}}, {}) == []


def test_extractor_exposes_only_bounded_metadata_and_normal_evidence(tmp_path):
    root = tmp_path / "bounded-path-traversal"
    root.mkdir()
    (root / "SKILL.md").write_text(
        "---\nname: bounded-path-traversal\n"
        "description: Reads a report file using an external file path "
